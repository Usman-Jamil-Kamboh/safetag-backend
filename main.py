...
# Use an absolute path so makedirs and StaticFiles always agree on the same directory
...
def _bg_init_db(retries: int = 15, delay: float = 3.0):
    ...
    # Step 0: download fonts (no-op if already cached)
    try:
        _ensure_fonts()
    except Exception as _fe:
        print(f"[Pasbaan] WARNING: _ensure_fonts failed: {_fe}", file=sys.stderr)
...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS qr_codes (
            ...
            vehicle_type TEXT      NOT NULL DEFAULT 'common',
    ...
    # 2d. Add vehicle_type column if missing (safe on older deployments).
    # NOT NULL DEFAULT 'common' backfills every existing row (e.g. the ~80
    # already-printed stickers) as 'common' automatically — no manual UPDATE needed.
    try:
        cur.execute("ALTER TABLE qr_codes ADD COLUMN vehicle_type TEXT NOT NULL DEFAULT 'common'")
        conn.commit()
    except Exception:
        conn.rollback()   # column already exists — that is fine
...
def db_insert_qr(qr_id: str, theme: str = "classic", vehicle_type: str = "common"):
    ...
    cur  = conn.cursor()
    cur.execute(
        "INSERT INTO qr_codes (qr_id, theme, vehicle_type) VALUES (%s, %s, %s)",
        (qr_id, theme, vehicle_type)
    )
    conn.commit()
    cur.close()
    release_db(conn)
...
def is_subscription_blocked(sub: dict) -> bool:
    """
    Decide if the contact page should be blocked for non-payment / suspension.

    True when:
      - admin manually suspended it (status == 'suspended'), OR
      - the period has lapsed without renewal — covers BOTH the free 6-month
        trial (status == 'trial') and a previously-activated paid period
        (status == 'active') once its expires_at has passed and nobody
...
def db_switch_plan(qr_id: str, new_plan: str):
    """Switch an existing subscription to a different plan (basic \u2194 premium)."""
    conn = get_db()
    cur  = conn.cursor()
    cur.execute(
        "UPDATE subscriptions SET plan=%s WHERE qr_id=%s",
        (new_plan, qr_id)
    )
    conn.commit()
    cur.close()
...
# Fonts are downloaded in the background init thread — not at import time
...
@app.get("/admin/bikes", response_class=HTMLResponse)
def admin_bikes(
    page: int = 1,
    per_page: int = 25,
    plan: str = "",
...
@app.get("/admin/common", response_class=HTMLResponse)
def admin_common(
    page: int = 1,
    per_page: int = 25,
    plan: str = "",
...
def scan_qr(qr_id: str, request: Request):
    ...
    if not owner_data:
        # First scan — send to the welcome/intro page before plan selection
        return RedirectResponse(url=f"/scan/{qr_id}/welcome", status_code=302)
...
@app.get("/scan/{qr_id}/welcome", response_class=HTMLResponse)
def welcome_page(qr_id: str):
    """One-time intro page shown before plan selection on the very first scan.
    Explains what Pasbaan is so a stranger scanning a marketing sticker
    understands the product before being asked to choose a plan."""
    record = db_get_record(qr_id)
    if not record:
    ...
    if record["owner_data"] is not None:
        # Already activated — no need to see the welcome page again
        return RedirectResponse(url=f"/scan/{qr_id}", status_code=302)
    return HTMLResponse(page_welcome(qr_id))
...
@app.post("/scan/{qr_id}/switch-to-basic", response_class=HTMLResponse)
async def switch_to_basic(qr_id: str, request: Request, pin: str = Form(...)):
    """PIN-verified downgrade from Premium to Basic."""
    record = db_get_record(qr_id)
    ...
    db_switch_plan(qr_id, "basic")
    return HTMLResponse(f"""<!DOCTYPE html><html lang="en">
<head><meta charset="UTF-8">{_css()}<title>Switched to Basic</title>
<meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body><div class="wrap">{_logo()}
<div class="card" style="text-align:center;padding:36px 20px;background:#f0fdf4;border:1.5px solid #86efac;">
  <div style="font-size:56px;margin-bottom:14px">🔵</div>
  <h2 style="font-size:20px;font-weight:800;color:#166534;margin-bottom:8px">Switched to Basic Plan</h2>
  <p style="font-size:14px;color:#15803d;line-height:1.7;">
    Your Pasbaan is now on the <strong>Basic plan</strong>.<br>
    Your phone numbers will be <strong>visible</strong> to scanners and<br>
    they can call you directly from your contact page.
  </p>
  <p style="font-size:12px;color:#6b7280;margin-top:12px">Sticker: <code>{qr_id}</code></p>
</div>
...
.vtype-card{{flex:1;padding:11px 12px;background:#111;border:1.5px solid #2a2a2a;border-radius:10px;
             cursor:pointer;transition:border-color .15s,color .15s;font-size:13px;color:#aaa;
             font-family:inherit}}
.vtype-card:hover{{border-color:#444}}
.vtype-card.selected{{border-color:#fff;color:#fff}}
...
<div class="main">
  ...
  <div class="card">
    ...
    <div style="margin-top:14px">
      ...
      <div style="display:flex;gap:10px;margin-top:2px" id="vtype-grid">
        <button type="button" class="vtype-card selected" data-vtype="common" onclick="selectVType('common')">⚪ Common</button>
...
function selectVType(vtype) {{
  ...
  document.querySelectorAll('.vtype-card').forEach(c => {{
    c.classList.toggle('selected', c.dataset.vtype === vtype);
  }});
}}
...
def page_welcome(qr_id: str) -> str:
    """First-scan welcome/intro page — shown once before plan selection.
    Explains what Pasbaan is and why it matters, with an EN/UR toggle.
    Kept short by design: purely a trust-building step before setup."""
    return f"""<!DOCTYPE html><html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Welcome — Pasbaan</title>
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
body{{
  font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
  background:#f0f0eb;min-height:100vh;padding:24px 16px 40px;color:#111;
}}
...
.logo-dot{{color:#b90a0a}}
.lang-toggle{{display:flex;background:#fff;border:1px solid #e0e0e0;border-radius:20px;padding:3px;gap:2px}}
.lang-btn{{border:none;background:transparent;padding:5px 12px;border-radius:16px;
           font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;color:#888}}
.lang-btn.active{{background:#111;color:#fff}}

/* Hero */
.hero{{background:#fff;border-radius:18px;padding:26px 22px;text-align:center;
       box-shadow:0 1px 4px rgba(0,0,0,.06);margin-bottom:14px}}
.hero-icon{{font-size:38px;margin-bottom:10px}}
.hero h1{{font-size:20px;font-weight:800;margin-bottom:8px;line-height:1.35}}
.hero p{{font-size:13px;color:#666;line-height:1.65}}
.qr-chip{{display:inline-block;margin-top:12px;padding:3px 12px;
          background:#f5f5f3;border:1px solid #e0e0e0;border-radius:20px;
          font-size:11px;color:#aaa;font-family:monospace}}

/* Feature rows */
.feat-card{{background:#fff;border-radius:14px;padding:14px 16px;margin-bottom:10px;
            display:flex;gap:12px;align-items:flex-start;box-shadow:0 1px 4px rgba(0,0,0,.05)}}
.feat-icon{{font-size:20px;flex-shrink:0;line-height:1;margin-top:1px}}
.feat-text h3{{font-size:13.5px;font-weight:700;margin-bottom:2px}}
.feat-text p{{font-size:12px;color:#777;line-height:1.5}}

/* CTA */
.cta-btn{{
  width:100%;padding:15px;border:none;border-radius:12px;margin-top:8px;
  font-size:15px;font-weight:700;cursor:pointer;font-family:inherit;
  background:linear-gradient(135deg,#7c3aed,#6d28d9);color:#fff;
  transition:transform .1s;letter-spacing:.01em;
}}
.cta-btn:active{{transform:scale(.97)}}
.cta-note{{font-size:11px;color:#aaa;text-align:center;margin-top:10px;line-height:1.6}}

/* RTL / Urdu handling */
[dir="rtl"] {{text-align:right}}
[dir="rtl"] .feat-card{{flex-direction:row-reverse}}
[dir="rtl"] .topbar{{flex-direction:row-reverse}}
.lang-block{{display:none}}
.lang-block.active-lang{{display:block}}
</style>
</head>
<body>
<div class="wrap">
  ...
  <div class="topbar">
    ...
    <div class="lang-toggle">
      ...
      <button type="button" class="lang-btn" id="btn-ur" onclick="setLang('ur')">اردو</button>
    </div>
  </div>

  <!-- ═══ ENGLISH ═══ -->
  <div id="block-en" class="lang-block active-lang">

    <div class="hero">
      <div class="hero-icon">🚗🛡️</div>
      <h1>Welcome — this vehicle is protected by Pasbaan</h1>
      <p>Pasbaan exists so that a parked vehicle is never truly unreachable in an emergency —
         without ever putting the owner's phone number at risk.</p>
    ...
    <div class="feat-card">
      ...
      <div class="feat-text">
        <h3>Reach the owner instantly</h3>
        <p>Scan the sticker, tap a button — no searching for contact details.</p>
      </div>
    </div>
    <div class="feat-card">
      <div class="feat-icon">🔒</div>
      <div class="feat-text">
        ...
        <p>On Premium, the owner's real number is never shown to anyone who scans.</p>
      </div>
    </div>
    <div class="feat-card">
      <div class="feat-icon">🆓</div>
      <div class="feat-text">
        <h3>Free to set up</h3>
        <p>Activating your own sticker takes under a minute — no cost for 6 months.</p>
      </div>
    </div>

    <a href="/scan/{qr_id}/choose-plan" style="text-decoration:none;display:block">
      <button type="button" class="cta-btn">This is my vehicle — Set It Up &rarr;</button>
    </a>
    <p class="cta-note">Takes less than a minute · Free for the first 6 months</p>
  </div>

  <!-- ═══ URDU ═══ -->
  <div id="block-ur" class="lang-block" dir="rtl">

    <div class="hero">
      <div class="hero-icon">🚗🛡️</div>
      <h1>خوش آمدید — یہ گاڑی Pasbaan کی حفاظت میں ہے</h1>
      <p>Pasbaan کا مقصد یہ ہے کہ ایمرجنسی میں کھڑی گاڑی کے مالک تک رابطہ ہمیشہ ممکن رہے —
         بغیر مالک کا فون نمبر ظاہر کیے۔</p>
      <span class="qr-chip">{qr_id}</span>
    </div>
    ...
    <div class="feat-card">
      <div class="feat-icon">📞</div>
      <div class="feat-text">
        <h3>مالک سے فوری رابطہ</h3>
        <p>سٹیکر سکین کریں، بٹن دبائیں — کوئی نمبر ڈھونڈنے کی ضرورت نہیں۔</p>
      </div>
    </div>
    <div class="feat-card">
      <div class="feat-icon">🔒</div>
      <div class="feat-text">
        <h3>پرائیویسی کا خاص خیال</h3>
        <p>پریمیم پلان میں مالک کا اصل نمبر کسی کو بھی نظر نہیں آتا۔</p>
      </div>
    </div>
    <div class="feat-card">
      <div class="feat-icon">🆓</div>
      <div class="feat-text">
        <h3>مفت رجسٹریشن</h3>
        <p>اپنا سٹیکر ایکٹو کرنے میں ایک منٹ سے بھی کم وقت لگتا ہے — 6 ماہ تک بالکل مفت۔</p>
      </div>
    </div>

    <a href="/scan/{qr_id}/choose-plan" style="text-decoration:none;display:block">
      <button type="button" class="cta-btn">یہ میری گاڑی ہے — ابھی سیٹ اپ کریں &rarr;</button>
    </a>
    <p class="cta-note">ایک منٹ سے کم وقت میں مکمل · پہلے 6 ماہ بالکل مفت</p>
  </div>

</div>
<script>
function setLang(lang){{
  document.getElementById('block-en').classList.toggle('active-lang', lang==='en');
  document.getElementById('block-ur').classList.toggle('active-lang', lang==='ur');
...
<div class="card" style="background:linear-gradient(135deg,#1e3a5f 0%,#1d4ed8 100%);border:none;padding:20px;">
  ...
  <button onclick="ecStart()" id="ec-start-btn" style="width:100%;display:flex;align-items:center;justify-content:center;gap:10px;padding:16px 18px;
    ...
    # Numpad HTML must be pre-built — backslashes aren't allowed in f-string expressions
    _spp_numpad = "\n".join(
        '<button type="button" onclick="spp(\'' + d + '\')" '
        'style="padding:14px;font-size:18px;font-weight:600;background:#f5f5f0;'
        'border:1.5px solid #e8e8e8;border-radius:11px;cursor:pointer">' + d + '</button>'
        for d in ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', '\u232b']
    )
    _mp_numpad = "\n".join(
        '<button type="button" onclick="mp(\'' + d + '\')" '
        'style="padding:14px;font-size:18px;font-weight:600;background:#f5f5f0;'
        'border:1.5px solid #e8e8e8;border-radius:11px;cursor:pointer">' + d + '</button>'
        for d in ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', '\u232b']
    )
    _dp_numpad = "\n".join(
        '<button type="button" onclick="dp(\'' + d + '\')" '
        'style="padding:14px;font-size:18px;font-weight:600;background:#f5f5f0;'
        'border:1.5px solid #e8e8e8;border-radius:11px;cursor:pointer">' + d + '</button>'
        for d in ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', '\u232b']
    )

    # -- Plan switch button (shown only to owner)
    if plan == "premium":
        plan_switch_html = (
            '<button onclick="document.getElementById(\'switch-plan-modal\').style.display=\'flex\'"' +
            ' style="width:100%;margin-top:6px;padding:11px;background:transparent;' +
            'border:1.5px solid #bfdbfe;border-radius:13px;font-size:13px;' +
            'color:#1d4ed8;cursor:pointer;font-family:inherit">' +
            ' \U0001f53d Switch to Basic Plan</button>'
        )
        plan_switch_modal_html = f"""<div id="switch-plan-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);
  ...
  <div style="background:#fff;border-radius:22px 22px 0 0;padding:28px 24px 36px;
    ...
    <div style="text-align:center;margin-bottom:18px">
      <div style="font-size:36px;margin-bottom:8px">\U0001f53d</div>
      <h3 style="font-size:17px;font-weight:700;margin-bottom:4px">Switch to Basic Plan?</h3>
      <p style="font-size:13px;color:#888">Enter your 4-digit PIN to confirm.<br>Your numbers will become visible to scanners.</p>
    </div>
    <form method="POST" action="/scan/{qr_id}/switch-to-basic" id="switch-plan-form">
      <div style="display:flex;justify-content:center;gap:8px;margin-bottom:18px">
        <input type="text" name="pin" id="sp-pin"
               style="width:160px;text-align:center;font-size:28px;font-weight:700;
                      letter-spacing:12px;padding:12px 14px;border:2px solid #1d4ed8;
                      border-radius:14px;background:#eff6ff;color:#111"
      ...
      <button type="submit" style="width:100%;padding:14px;background:#1d4ed8;color:#fff;
              border:none;border-radius:13px;font-size:15px;font-weight:700;
              cursor:pointer;font-family:inherit">
        \U0001f53d Confirm Switch to Basic
      </button>
    </form>
    <button onclick="document.getElementById('switch-plan-modal').style.display='none'"
...
<script>
const sppin = document.getElementById('sp-pin');
function spp(v){{if(v==='\u232b'){{sppin.value=sppin.value.slice(0,-1);return;}}if(sppin.value.length<4) sppin.value+=v;}}
sppin.addEventListener('input',()=>{{sppin.value=sppin.value.replace(/\\D/g,'').slice(0,4);}});
sppin.addEventListener('input',()=>{{if(sppin.value.length===4) document.getElementById('switch-plan-form').requestSubmit();}});
</script>"""
    else:  # basic plan
        plan_switch_html = (
...
def page_pin_prompt(qr_id: str) -> str:
    ...
    _pad_numpad = "\n".join(
        '<button type="button" onclick="padPress(\'' + d + '\')" '
        'style="padding:16px;font-size:20px;font-weight:600;background:#f5f5f0;'
        'border:1.5px solid #e0e0e0;border-radius:12px;cursor:pointer">' + d + '</button>'
        for d in ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', '\u232b']
    )
    return f"""<!DOCTYPE html><html lang="en">
...
def page_subscription_expired(qr_id: str, suspended: bool = False) -> str:
    """
    Shown on the contact page when the subscription is blocked:
    either the free trial / paid period lapsed without renewal, or
    the admin manually suspended it. Renewal is via manual WhatsApp + JazzCash/Easypaisa,
    matching the rest of Pasbaan's payment flow.
    """
    ...
    if _wa.startswith("0"):
        ...
        _wa = "92" + _wa
    wa_msg = f"Hi, I want to renew my Pasbaan subscription for sticker {qr_id}"
    wa_link = f"https://wa.me/{_wa}?text={wa_msg.replace(' ', '%20').replace(',', '%2C')}"

    heading = "Subscription Suspended" if suspended else "Subscription Expired"
    body = (
        "This Pasbaan sticker's subscription has been suspended by the admin."
        if suspended else
        "This Pasbaan sticker's free trial or subscription period has ended without renewal."
    )

    return f"""<!DOCTYPE html><html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
{_css()}<title>{heading} — {qr_id}</title></head>
<body><div class="wrap">{_logo()}
<div class="card" style="text-align:center;padding:40px 20px;border:1.5px solid #fecaca;background:#fff5f5">
  <div style="font-size:64px;margin-bottom:16px">⏳</div>
  <h2 style="font-size:22px;font-weight:700;color:#9f1239;margin-bottom:10px">{heading}</h2>
...
<div class="card" style="margin-top:12px;text-align:center">
  <p style="font-size:13px;color:#555;margin-bottom:14px">
    Are you the vehicle owner? Renew your Pasbaan subscription to restore this page.
  </p>
  <a href="{wa_link}" target="_blank"
    style="display:block;width:100%;padding:13px;background:#16a34a;color:#fff;text-decoration:none;
           border-radius:13px;font-size:15px;font-weight:700;box-shadow:0 4px 14px rgba(22,163,74,.3)">
    💬 Renew via WhatsApp
  </a>
</div>
</div></body></html>"""
...
def page_deactivated(qr_id: str) -> str:
    ...
    _rp_numpad = "\n".join(
        '<button type="button" onclick="rp(\'' + d + '\')" '
        'style="padding:14px;font-size:18px;font-weight:600;background:#f5f5f0;'
        'border:1.5px solid #e8e8e8;border-radius:11px;cursor:pointer">' + d + '</button>'
        for d in ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', '\u232b']
    )
